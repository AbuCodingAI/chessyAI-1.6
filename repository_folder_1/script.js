...
const gameState = {
  ...
  gameMode: 'local',
  playerColor: 'white', // Player's color (white or black)
  gameOver: false,
  whiteTime: 600,
  blackTime: 600,
  timerInterval: null,
...
function handleSquareClick(row, col) {
  ...
  if (gameState.gameMode !== 'local' && gameState.currentPlayer !== gameState.playerColor) {
    return;
  }
...
function makeMove(fromRow, fromCol, toRow, toCol) {
  ...
  // Force immediate board update BEFORE checking game end
  renderBoard();
  updateGameStatus();
  updateMoveHistory();
  updateCapturedPieces();
  ...
  // Check for game end conditions AFTER rendering
...
function endGame(winner, reason = 'checkmate') {
  ...
  // Show game over message (delay to let final move render)
  setTimeout(() => {
    let message = '';
    ...
    alert(message);
  }, 800); // Increased delay to let final move render
}
...
document.getElementById('new-game').addEventListener('click', () => {
  ...
  if (playerColor === 'black' && gameState.gameMode.startsWith('ai-')) {
    setTimeout(() => {
      makeAIMove();
    }, 500);
  }
  ...
  btn.textContent = playerColor === 'black' ? '✓ AI moves first!' : '✓ Game Started!';
...
function initPuzzleSystem() {
  // Load puzzle engine script
  const script = document.createElement('script');
  script.src = 'puzzle-engine.js';
  script.onload = () => {
...
function loadRandomPuzzle() {
  ...
  const puzzle = puzzleEngine.loadRandomPuzzle();
  if (!puzzle) return;
  ...
  loadPuzzlePosition(puzzle.fen);

  // Update UI
  document.getElementById('puzzle-theme').textContent = puzzle.theme;
...
// Handle puzzle move
function handlePuzzleMove(fromRow, fromCol, toRow, toCol) {
  const piece = gameState.board[fromRow][fromCol];
  if (!piece) return false;
  ...
  const move = files[fromCol] + ranks[fromRow] + files[toCol] + ranks[toRow];

  // Validate move with puzzle engine
  const result = puzzleEngine.validateMove(move);
  ...
  if (result.correct) {
    ...
    if (result.complete) {
      // Puzzle solved!
      handlePuzzleSolved(result.timeSpent);
    }
  } else {
    // Wrong move
    const feedback = document.getElementById('puzzle-feedback');
...
function handlePuzzleSolved(timeSpent) {
  // Update stats
  ...
  // Update rating based on puzzle difficulty
  const puzzleDiff = puzzleEngine.currentPuzzle.difficulty;
  const ratingChange = calculatePuzzleRatingChange(stats.rating, puzzleDiff, true);
  ...
  feedback.innerHTML = `
    🎉 Puzzle Solved!<br>
    Time: ${timeSpent}s<br>
    Rating: ${stats.rating} (${ratingChange > 0 ? '+' : ''}${ratingChange})
  `;
  feedback.className = 'puzzle-feedback success';
...
function calculatePuzzleRatingChange(userRating, puzzleDiff, correct) {
  const K = 32;
  const expected = 1 / (1 + Math.pow(10, (puzzleDiff - userRating) / 400));
  const actual = correct ? 1 : 0;
  return Math.round(K * (actual - expected));
}

// Update puzzle progress display
function updatePuzzleProgress() {
  ...
  document.getElementById('user-puzzle-rating').textContent = stats.rating;
  document.getElementById('puzzles-solved').textContent = stats.solved;
  document.getElementById('puzzle-streak').textContent = stats.streak;
}

// Show puzzle hint
function showPuzzleHint() {
  if (!puzzleEngine || !puzzleEngine.currentPuzzle) return;
...
function showPuzzleSolution() {
  ...
  const solution = puzzleEngine.showSolution();
  const feedback = document.getElementById('puzzle-feedback');
  feedback.innerHTML = `
    <strong>Solution:</strong><br>
    Moves: ${solution.moves.join(', ')}<br>
    ${solution.explanation}
  `;
  feedback.className = 'puzzle-feedback solution';
}

// Event listeners for puzzle controls
document.getElementById('next-puzzle')?.addEventListener('click', loadRandomPuzzle);
document.getElementById('puzzle-hint')?.addEventListener('click', showPuzzleHint);
document.getElementById('puzzle-solution')?.addEventListener('click', showPuzzleSolution);

// Initialize puzzle system when puzzles view is opened
document.querySelector('[data-view="puzzles"]')?.addEventListener('click', () => {
  if (!puzzleEngine) {
    initPuzzleSystem();
  }
});
