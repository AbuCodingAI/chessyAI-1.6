...
def get_move():
    ...
    try:
        ...
        # DEBUG: Print received board
        print("\n=== Board Reconstruction Debug ===")
        for row in range(8):
            print(f"Row {row}: {board_array[row]}")
        ...
        for row in range(8):
            for col in range(8):
                ...
                if piece_symbol:
                    ...
                    # Frontend sends board[row][col] where:
                    # row 0 = rank 8 (black's back rank)
                    # row 7 = rank 1 (white's back rank)
                    # chess.square(file, rank) expects rank 0 = rank 1
                    square = chess.square(col, 7 - row)
                    board.set_piece_at(square, chess.Piece(piece_type, color))
        
        print(f"Reconstructed FEN: {board.fen()}")
        print(f"Legal moves: {list(board.legal_moves)[:5]}")
        print("=" * 40)
