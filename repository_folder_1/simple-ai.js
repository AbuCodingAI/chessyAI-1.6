...
class ChessyAI {
    ...
    /**
     * NOOB (ELO 100)
     * 70% random moves, 30% random captures
     * Designed to be beatable for absolute beginners
     */
    noob(moves) {
        ...
        // 30% chance: try to capture something (but randomly, not best capture)
     ...
     * ~2500 ELO (IM level)
     * NOTE: Neural network trained by Stockfish, not using Stockfish directly
     */
    async chessy13(fen) {
        ...
        // Step 2: Use neural network
        // TODO: Integrate with neural-ai/chess_engine_deep_search.py
        // TEMPORARY: Using Stockfish depth 20 as placeholder
    ...
    async chessy14(fen) {
        ...
        // Step 2: Check for mate-in-2 using Stockfish (safety check)
        ...
        // Step 3: Use neural network
        // TODO: Integrate with neural-ai/chess_engine_quiescence.py
        // TEMPORARY: Using Stockfish depth 25 as placeholder
        return await this.stockfishMove(fen, 25);
    }
     ...
     * @param {number} mateDepth - Mate depth to search (1 for M1, 2 for M2, etc.)
     * @returns {Promise<object|null>} - Move object if mate found, null otherwise
     */
        ...
        return new Promise((resolve) => {
            ...
            stockfish.stdout.on('data', (data) => {
                ...
                if (match) {
                    ...
                    // Only return move if mate was actually found
                    if (mateFound && bestMove) {
                        const game = new Chess(fen);
                        const move = game.move({
                            from: bestMove.substring(0, 2),
                            to: bestMove.substring(2, 4),
