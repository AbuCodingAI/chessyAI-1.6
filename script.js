...
function initBoard() {
  ...
  gameState.lastMove = null;  // Reset last move
...
function makeMove(fromRow, fromCol, toRow, toCol) {
  ...
  // Store last move for display in game end prompt
  gameState.lastMove = {
...
// Show auth modal
function showAuthModal(isLogin = false) {
  ...
  if (isLogin) {
    ...
    toggleText.innerHTML = "Don't have an account? <a href='#' id='toggle-signup'>Sign Up</a>";
  } else {
    title.textContent = 'Sign Up';
    submit.textContent = 'Sign Up';
    toggleText.innerHTML = "Already have an account? <a href='#' id='toggle-login'>Log In</a>";
  }
...
async function signUp(email, password) {
  try {
    ...
    // Create user profile
    gameState.profile.username = email.split('@')[0];
    localStorage.setItem('chessProfile', JSON.stringify(gameState.profile));
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  document.addEventListener('click', (e) => {
    if (e.target.id === 'toggle-login') {
      e.preventDefault();
      showAuthModal(true);
    } else if (e.target.id === 'toggle-signup') {
      e.preventDefault();
      showAuthModal(false);
    }
  });
...
// ============================================
// AI MOVE REQUEST FUNCTIONS
// ============================================

// Get AI move from backend server
async function getAIMove(fen, aiType) {
  try {
    // Map game mode to AI personality
    const personalityMap = {
      ...
      'ai-gm': 'gm',
      'ai-supergm': 'supergm',
      'ai-chessy13': 'chessy13',
      'ai-chessy14': 'chessy14',
      'ai-randomguy': 'randomguy',
      'ai-mystery': 'mystery',
      'ai-randy': 'randy',
      'ai-antiguess': 'antiguess',
      'ai-trashtalker': 'trashtalker',
      'ai-chocker': 'chocker'
    };

    const personality = personalityMap[aiType] || 'noob';
    ...
    const response = await fetch('/api/ai-move', {
      ...
      body: JSON.stringify({ fen, personality })
    });
    ...
    if (data.redirect === 'chocker') {
      alert('⚠️ Chocker requires Python backend!\n\n' + data.launchCommand);
      return null;
    }
...
function boardToFEN() {
  ...
  fen += ' ' + gameState.fiftyMoveCounter + ' ' + Math.ceil(gameState.moveCount / 2);
...
// Add training metric entry
function addTrainingMetric(epoch, trainLoss, valLoss, isOverfitting = false) {
  const gap = (valLoss - trainLoss).toFixed(6);

  const metric = {
    epoch,
    trainLoss: parseFloat(trainLoss).toFixed(6),
    valLoss: parseFloat(valLoss).toFixed(6),
    gap,
    isOverfitting,
    timestamp: new Date().toLocaleTimeString()
  };

  trainingMetrics.data.push(metric);

  // Update best validation loss
  if (valLoss < trainingMetrics.bestValLoss) {
    trainingMetrics.bestValLoss = valLoss;
    trainingMetrics.bestEpoch = epoch;
  }

  // Count overfitting epochs
  if (isOverfitting) {
    trainingMetrics.overfittingCount++;
  }
...
function updateTrainingMetricsDisplay() {
  ...
  if (trainingMetrics.data.length === 0) {
    tbody.innerHTML = `
      <tr class="empty-state">
        <td colspan="6" style="text-align: center; padding: 2rem; color: #999;">
          No training data yet. Start a training session to see metrics.
        </td>
      </tr>
    `;
  ...
  // Add metric rows
  trainingMetrics.data.forEach((metric, index) => {
    const row = document.createElement('tr');

    // Highlight best epoch and overfitting rows
    if (metric.epoch === trainingMetrics.bestEpoch) {
      row.classList.add('best-epoch-row');
    }
    if (metric.isOverfitting) {
      row.classList.add('overfitting-row');
    }

    const overfittingBadge = metric.isOverfitting
      ? '<span class="overfitting-badge">⚠️ YES</span>'
      : '<span class="normal-badge">✅ NO</span>';

    const status = metric.epoch === trainingMetrics.bestEpoch
      ? '🏆 Best'
      : metric.isOverfitting
        ? '⚠️ Overfitting'
        : '✅ Good';

    row.innerHTML = `
      <td>${metric.epoch}</td>
      <td>${metric.trainLoss}</td>
  ...
  if (summary) {
    ...
    document.getElementById('total-epochs').textContent = trainingMetrics.data.length;
    document.getElementById('overfitting-epochs').textContent = trainingMetrics.overfittingCount;
    document.getElementById('best-val-loss').textContent = trainingMetrics.bestValLoss.toFixed(6);

    const status = trainingMetrics.overfittingCount > 0 ? '⚠️ Overfitting Detected' : '✅ Training Healthy';
...
function startTrainingMonitor() {
  trainingMetrics.isMonitoring = true;
  trainingMetrics.data = [];
  trainingMetrics.bestValLoss = Infinity;
  trainingMetrics.bestEpoch = 0;
  trainingMetrics.overfittingCount = 0;

  updateTrainingMetricsDisplay();

  alert('✅ Training monitor started!\n\nMetrics will appear here as training progresses.\n\nTo add metrics, call:\naddTrainingMetric(epoch, trainLoss, valLoss, isOverfitting)');
}

// Clear training metrics
function clearTrainingMetrics() {
  if (confirm('Clear all training metrics?')) {
    trainingMetrics.data = [];
    trainingMetrics.bestValLoss = Infinity;
    trainingMetrics.bestEpoch = 0;
    trainingMetrics.overfittingCount = 0;
    updateTrainingMetricsDisplay();
    alert('✅ Training metrics cleared');
  }
}
...
function downloadMetricsCSV() {
  ...
  // Create CSV header
  let csv = 'Epoch,Train Loss,Val Loss,Gap,Overfitting,Timestamp\n';

  // Add data rows
  trainingMetrics.data.forEach(metric => {
    csv += `${metric.epoch},${metric.trainLoss},${metric.valLoss},${metric.gap},${metric.isOverfitting ? 'YES' : 'NO'},${metric.timestamp}\n`;
  });
  ...
  a.href = url;
  a.download = `training-metrics-${new Date().toISOString().slice(0, 10)}.csv`;
  document.body.appendChild(a);
  a.click();
  window.URL.revokeObjectURL(url);
  document.body.removeChild(a);

  alert(`✅ Downloaded ${trainingMetrics.data.length} metrics to CSV`);
}

// Simulate training for testing
function simulateTraining() {
  console.log('🎓 Simulating training with overfitting...');

  const trainLosses = [0.5, 0.4, 0.3, 0.25, 0.22, 0.20, 0.19, 0.18, 0.17, 0.16];
  const valLosses = [0.5, 0.4, 0.35, 0.35, 0.36, 0.37, 0.38, 0.39, 0.40, 0.41];

  startTrainingMonitor();

  trainLosses.forEach((trainLoss, epoch) => {
    const valLoss = valLosses[epoch];
    const isOverfitting = epoch > 3 && valLoss > valLosses[3];

    setTimeout(() => {
      addTrainingMetric(epoch, trainLoss, valLoss, isOverfitting);
      console.log(`Epoch ${epoch}: train=${trainLoss.toFixed(4)}, val=${valLoss.toFixed(4)}, overfitting=${isOverfitting}`);
    }, epoch * 500);
  });
}

// Export functions for external use
window.addTrainingMetric = addTrainingMetric;
window.startTrainingMonitor = startTrainingMonitor;
window.clearTrainingMetrics = clearTrainingMetrics;
window.downloadMetricsCSV = downloadMetricsCSV;
window.simulateTraining = simulateTraining;
